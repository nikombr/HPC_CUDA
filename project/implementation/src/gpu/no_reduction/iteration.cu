...
__global__ void iteration_inner(double *** u, double *** uold, double *** f, int N, int iter_max) {
...
void iteration(double *** u, double *** uold, double *** f, int N, int iter_max) {
    ...
    // Do iteration
    iteration_inner<<<dimGrid, dimBlock>>>(u, uold, f, N, iter_max);
