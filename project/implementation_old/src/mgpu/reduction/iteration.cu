...
__global__ void iteration_inner(double *** u, double *** uold, double *** f, int N, int width, double*res, double delta2, double frac) {
    double val = 0.0;
...
void iteration(double *** u, double *** uold, double *** f, int N, int width, double*sum) {
    ...
    //dim3 dimGrid(1,1,1);
    iteration_inner<<<dimGrid, dimBlock>>>(u, uold, f, N, width, sum, delta2, frac);
