...
do
    NCCL_SOCKET_NTHREADS=16 mpirun -npernode 2 ./${EXECUTEFOLDER}mgpu_no_reduction $N $ITER $TOLERANCE $START_T >> $FILE_NO_REDUCTION
    ...
    NCCL_SOCKET_NTHREADS=16 mpirun -npernode 2 ./${EXECUTEFOLDER}mgpu_reduction $N $ITER $TOLERANCE $START_T >> $FILE_REDUCTION
done
exit 0
